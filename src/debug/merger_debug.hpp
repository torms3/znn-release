...
public:
    void run()
    {
        ...
        for ( std::size_t i = 0; i < op_->n_iters ; ++i )
        {
            ...
            // applying MALIS
            result = merger(s->labels, s->inputs, s->masks);

            // accumulate volume
            const std::string& str = op_->save_path + op_->outname;

            accumulate_volumes(s->inputs,   str + mstr + ".affin");
            accumulate_volumes(s->labels,   str + mstr + ".truth");
            accumulate_volumes(mpair.first, str + mstr + ".malis");
